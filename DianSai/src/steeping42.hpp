...
const double Once_Circle_Distance = 40.0;
...
*/
    Steeping42_Command_Status Steeping_Init(HardwareSerial *serial, uint8_t id)
    {
        ...
        this->Change_Zero_Parameter(600);
        this->Back_To_Zero_Location(0x02,true);
...
/*
    位置控制模式
*/
    Steeping42_Command_Status Location_Mode_Cmd(uint16_t speed, uint32_t location, uint8_t acceleration = 0x00, bool loaction_mode = false, bool dual_machine = true)
    {
        uint8_t direction = location>0? 0x00: 0x01;
...
/*
    修改电机的回零的参数
*/
    Steeping42_Command_Status Change_Zero_Parameter(uint16_t speed, uint8_t store = 0x01,uint8_t zero_mode = 0x02, uint8_t direction = 0x00)
    {
