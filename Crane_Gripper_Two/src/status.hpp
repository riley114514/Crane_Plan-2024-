...
void Task_Status_Checking(void *prsm)
{
    while (1)
    {
        if (eps_now_e.gripper_two_next_state == gripper_two_next_start_scan)
        {
            gripper_two.Gripper_Set_Z_Location(240);
            gripper_two.Gripper_Set_Y_Location(1800);
            eps_now_e.gripper_two_next_state == gripper_two_next_stop;
            esp_now_community.Send_Message();
        }
        else if (eps_now_e.gripper_two_next_state == gripper_two_next_pick_start)
        {
            gripper_two.Gripper_Set_Y_Location(Weight_Location_Y[eps_now_e.gripper_two_pick_num / 16 - 1][eps_now_e.gripper_two_pick_num % 16 - 1]);
            gripper_two.servo->Set_Servo_Angle(150);
            gripper_two.Gripper_Set_Z_Location(5);
            gripper_two.servo->Set_Servo_Angle(80);
            delay(500);
            gripper_two.Gripper_Set_Z_Location(240);
            gripper_two.Gripper_Set_Y_Location(1755);
            eps_now_e.gripper_two_next_state = gripper_two_next_pick_finish;
        ...
        else if (eps_now_e.gripper_two_next_state == gripper_two_next_set_start)
        {
            ...
            eps_now_e.gripper_two_next_state = gripper_two_next_set_finish;
